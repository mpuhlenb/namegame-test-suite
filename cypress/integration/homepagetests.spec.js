...
context('HomePage', () => {
    ...
    it('Question is a sentence', () => {
        cy.get('h1.text-center').should(($textcenter) => {
            const sentence = $textcenter.text()
            expect(sentence).to.match(/^Who is /)
        })

        cy.get('#name').should(($fullname) => {
    ...
    it('Attempts/Tries counter increments after selecting a photo', () => {
        cy.get('.attempts').invoke('text').then((initialTries) => {
            ...
            cy.get('.attempts').invoke('text').should((nextAttempt) => {
    ...
    it('All counter increment when correct photo is found x times', () => {
        ...
        cy.get('span.correct').invoke('text').then((initialCorrect) => {
            cy.get('.streak').invoke('text').then((initialStreak) => {
                cy.get('.attempts').invoke('text').then((initialAttempts) => {
                    // Set a streak and check incremented by streak
                    createASimpleStreak(streak)
                    cy.get('.streak').should('not.be.equal', initialStreak)
                        .invoke('text').then((finalStreak) => {
                            cy.get('span.correct').invoke('text').then((finalCorrect) => {
                                cy.get('.attempts').invoke('text').then((finalAttempt) => {
                                    // All counters should have increased by streak amount
                                    expect(Number(finalCorrect)).to.eq(Number(initialCorrect) + streak)
                                    expect(Number(finalStreak)).to.eq(Number(initialStreak) + streak)
                                    expect(Number(finalAttempt)).to.eq(Number(initialAttempts) + streak)
                                })
                            })
        ...
        cy.get('span.correct').invoke('text').then((initialCorrect) => {
            cy.get('.streak').invoke('text').then((initialStreak) => {
                ...
                cy.get('.attempts').invoke('text').then((initialAttempts) => {
                    ...
                    cy.get('.streak').invoke('text').then((endofStreak) => {
                        cy.get('span.correct').invoke('text').then((finalCorrect) => {
                            cy.get('.attempts').invoke('text').then((lastAttempt) => {
    ...
    it('Photos and names update when correct guess made', () => {
        ...
        cy.get('.gallery').should('not.be.empty')
        cy.get('div.photo').children('div.name').invoke('text').as('names')
        cy.get('div.photo').then((initialPhotos) => {
            cy.get('@names').then((initialNames) => {
                clickCorrectPhoto()
                //After correct photo get photo array and names and ensure not equal to initial
                cy.get('.photo.correct').should('not.exist')
                cy.get('.photo').should('not.be.empty')
                cy.get('div.photo').then((finalPhotos) => {
                    cy.get('div.photo').children('div.name').invoke('text').then((finalNames) => {
    ...
    it('The proper decorator appears for wrong guesses', () => {
        cy.get('.photo.wrong').should('not.exist')
        cy.get('#name').attribute('data-n').then((datan) => {
            ...
            cy.get('.photo').children('div.shade').eq(Number(datan) + indexer)
                .should('have.css', 'background-color').and('equal', 'rgba(184, 20, 20, 0)')
            cy.get('.photo').children('div.name').eq(Number(datan) + indexer)
                .should('have.css', 'top').then((beforeText) => {
                    cy.get('.photo').eq(Number(datan) + indexer).click()
                    cy.get('.photo').children('div.shade').eq(Number(datan) + indexer)
                        .should('have.css', 'background-color').and('equal', 'rgba(184, 20, 20, 0.5)')
                    cy.get('.photo').children('div.name').eq(Number(datan) + indexer)
                        .should('have.css', 'top').then((afterText) => {
    ...
    it('The proper decorator appears for correct guesses', () => {
        cy.get('#name').attribute('data-n').then((datan) => {
            cy.get('.photo').children('div.shade').eq(datan).as('correctoverlay')
            cy.get('.photo').children('div.name').eq(datan).as('correctname')
            cy.get('@correctoverlay').should('have.css', 'background-color').and('equal', 'rgba(184, 20, 20, 0)')
            cy.get('@correctname').should('have.css', 'top').then((beforeText) => {
                cy.get('@correctoverlay').click()
                cy.get('@correctoverlay').should('have.css', 'background-color').and('equal', 'rgba(20, 184, 20, 0.5)')
                cy.get('@correctname').should('have.css', 'top').then((afterText) => {
                    expect(beforeText).to.not.be.equal(afterText)
                })
            })
        })
